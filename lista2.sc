...
//zadanie 4a
val k1 = List(-2,-1,0,1,2)
val List(_,_,x,_,_) = k1
...
//zadanie 5
def initSegment [A](first:List[A],second:List[A]):Boolean =
  (first,second) match{
    case (Nil,_) => true
    case (_,Nil) => false
    case _ => if (first.head == second.head) initSegment(first.tail, second.tail)
    else false
  }
...
//zadanie 6
def replaceNth[A](list:List[A],pos:Int,elem:A):List[A] =
  (list,pos) match {
    case (Nil,_) => Nil
    case (head :: tail,0) => elem :: tail
    case (head :: tail,_) => head :: replaceNth(tail,pos-1,elem)
  }
